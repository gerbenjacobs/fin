...
func main() {
	...
	stocks := trading212.Accumulate(cfg.Splits, events)

	log.Info("Completed aggregation.")
...
func writeOutput(cfg fin.Config, outputName string, output interface{}) string {
	fn := fmt.Sprintf("acc_%s_%s.csv", cfg.Output, strings.ToLower(outputName))
	if outputName == outputNone {
		fn = "acc_" + cfg.Output + ".csv"
	}
